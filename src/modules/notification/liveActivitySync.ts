...
import {
    ...
    type LiveActivityCapabilities,
    type LiveActivityPushTokenEvent,
    type LiveActivityStateChangeEvent,
} from "./liveActivity";

const LIVE_ACTIVITY_TYPE = "NoLateDepartureAttributes" as const;
const LIVE_ACTIVITY_SCHEMA_VERSION = 1 as const;
const LIVE_ACTIVITY_REGISTRATION_RETRY_DELAYS_MS = [
    1_500,
    4_000,
    15_000,
] as const;
const LIVE_ACTIVITY_RETIREMENT_RETRY_DELAYS_MS = [
    1_500,
    4_000,
    15_000,
] as const;
...
export type LiveActivitySyncDependencies = {
    ...
    registerStartToken: (payload: RegisterLiveActivityStartTokenPayload) => Promise<void>;
    retireStartToken: (deviceId: string) => Promise<void>;
    registerUpdateToken: (
        activityId: string,
        payload: RegisterLiveActivityUpdateTokenPayload,
    ) => Promise<void>;
    retireActivity: (activityId: string, payload: RetireLiveActivityPayload) => Promise<void>;
    end: typeof endLiveActivity;
    endAll: typeof endAllLiveActivities;
...
function scheduleIdAsNumber(scheduleId: string): number | undefined {
    if (!/^[1-9]\d*$/.test(scheduleId)) return undefined;
    const value = Number(scheduleId);
    return Number.isSafeInteger(value) ? value : undefined;
}

function validMemberId(value: unknown): value is number {
    return typeof value === "number" && Number.isSafeInteger(value) && value > 0;
}

function nativeEventKey(event: LiveActivityPushTokenEvent): string | undefined {
    if (event.kind === "pushToStart") return "start";
    return event.activityId ? `update:${event.activityId}` : undefined;
}

function startRegistrationValue(
    pushToStartToken: string,
    appearance: LiveActivityAppearance,
): string {
...
export function createLiveActivitySyncCoordinator(
    ...
    const pendingActivityRetirements = new Map<string, PendingActivityRetirement>();
    const registrationRetryDelaysMs = (
        dependencies.registrationRetryDelaysMs ?? LIVE_ACTIVITY_REGISTRATION_RETRY_DELAYS_MS
    ).filter((delay) => Number.isFinite(delay) && delay >= 0);
    const retirementRetryDelaysMs = (
        dependencies.retirementRetryDelaysMs ?? LIVE_ACTIVITY_RETIREMENT_RETRY_DELAYS_MS
    ).filter((delay) => Number.isFinite(delay) && delay >= 0);
    ...
    const cancelRetryWait = (lane: SyncLane) => {
    ...
    const rememberActivity = (
        ...
        const latestGeneration = candidate.latestGenerationByScheduleId.get(activity.scheduleId);
        if (latestGeneration !== undefined && activity.generation < latestGeneration) return false;
        if (latestGeneration === undefined || activity.generation > latestGeneration) {
            candidate.latestGenerationByScheduleId.set(activity.scheduleId, activity.generation);
        }
    ...
    const syncUpdateToken = (
        ...
        const latestGeneration = candidate.latestGenerationByScheduleId.get(scheduleId);
        if (latestGeneration !== undefined && generation < latestGeneration) return Promise.resolve();
        const numericScheduleId = scheduleIdAsNumber(scheduleId);
        if (!numericScheduleId) return Promise.resolve();
        return enqueue(candidate, `update:${activityId}`, `${generation}:${updateToken}`, () => (
    ...
    const retireActivityFromServer = (
        ...
        return (updateLane?.tail ?? Promise.resolve()).then(() =>
            enqueue(candidate, `retire:${activityId}`, scheduleId, async () => {
    ...
    const handlePushToken = (
        ...
        if (!event.activityId || !event.scheduleId || !event.recipientMemberId) return;
        const activity = {
    ...
    const handleStateChange = (
        ...
        if (!isCurrent(candidate) || event.recipientMemberId !== candidate.memberId) return;
        if (event.state === "active" || event.state === "stale") {
            if (event.state === "active") candidate.terminalActivityIds.delete(event.activityId);
            const existing = candidate.activeById.get(event.activityId);
            if (existing) rememberActivity(candidate, existing);
            return;
        }
        retireActivityFromServer(candidate, event.activityId, event.scheduleId)
            .catch(() => undefined);
    };
    ...
    const synchronizeSnapshot = (
        ...
        const request = Promise.all([
            ...
            const pendingForCandidate = [...pendingActivityRetirements.values()].filter(
                (pending) => pending.memberId === candidate.memberId &&
            ...
            for (const activity of activities) {
                if (activity.recipientMemberId !== candidate.memberId) continue;
                if (pendingActivityIds.has(activity.activityId)) continue;
                if (activity.generation < (latestGenerationByScheduleId.get(activity.scheduleId) ?? 0)) {
                    continue;
                }
                ...
                if (
                    ...
                    (candidate.nativeEventRevisionByKey.get(key) ?? -1) <= eventRevisionAtStart
                ) {
                    operations.push(syncUpdateToken(
                        candidate,
                        activity.activityId,
                        activity.scheduleId,
                        activity.generation,
            ...
            for (const pending of pendingForCandidate) {
                operations.push(retireActivityFromServer(
                    candidate,
                    pending.activityId,
                    pending.scheduleId,
                ));
            ...
            if (!isCurrent(candidate)) return;
            const reconciledStartToken = preferredStartToken ??
            ...
            if (
                ...
                (candidate.nativeEventRevisionByKey.get("start") ?? -1) <= eventRevisionAtStart
            ) {
            ...
            if (candidate.snapshotFlight === request) candidate.snapshotFlight = undefined;
        });
    ...
    const activate = async (memberId: number): Promise<void> => {
        ...
        if (session) {
            ...
            if (
                ...
                throw new Error(cleanup.reason ?? "Live Activity account-switch cleanup failed.");
            }
            throw new Error(
    ...
    const performClearForAccount = async (memberId?: number): Promise<void> => {
        const previous = session;
        const cleanupMemberId = memberId ?? previous?.memberId ?? cleanupRequiredMemberId;
        if (
            cleanupRequiredMemberId !== undefined &&
        ...
        previous?.activeById.forEach((activity, id) => activities.set(id, activity));
        ...
        if (deviceId && validMemberId(cleanupMemberId)) {
            ...
            const remoteRetirements: Promise<void>[] = [retryBounded(
            ...
            for (const activity of activities.values()) {
                ...
                remoteRetirements.push(retryBounded(async () => {
            ...
            results.forEach((result) => {
                if (result.status === "rejected") {
                    report("[live-activity] logout server retirement failed", result.reason);
        ...
        const nativeCleanupFailed = nativeResult.supported &&
            !nativeResult.applied &&
            nativeResult.reason !== "NOT_FOUND";

        if (nativeCleanupFailed) {
            throw new Error(nativeResult.reason ?? "Live Activity native cleanup failed.");
        }
    ...
    const endSchedule = async (scheduleId: string, memberId?: number): Promise<void> => {
        const candidate = session;
        if (!candidate || !isCurrent(candidate)) return;
        if (memberId !== undefined && candidate.memberId !== memberId) return;
        if (!scheduleIdAsNumber(scheduleId) || candidate.endedScheduleIds.has(scheduleId)) return;
        ...
        try {
            ...
            await Promise.all([...ids].map((activityId) => (
    ...
    return { activate, resume, setAppearance, pause, clearForAccount, endSchedule };
}
...
const productionCoordinator = createLiveActivitySyncCoordinator({
    getDeviceId: () => {
        const { getOrCreatePushDeviceId } = require("./pushDeviceIdentity") as (
            typeof import("./pushDeviceIdentity")
        );
    ...
    registerUpdateToken: (activityId, payload) => (
        getLiveActivityNotificationApi().registerLiveActivityUpdateToken(activityId, payload)
    ),
...
export async function clearLiveActivitiesForAccountCleanup(): Promise<void> {
    const { getAuthMember } = require("../auth/authStorage") as (
        typeof import("../auth/authStorage")
    );
