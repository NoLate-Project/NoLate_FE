...
describe("iOS NoLate custom alarm native contract", () => {
    const scheduler = read("modules/nolate-alarm/ios/NoLateCustomAlarmNotification.swift");
    const coordinator = read("modules/nolate-alarm/ios/NoLateAlarmCoordinator.swift");
    ...
    const foregroundRouter = read("src/modules/notification/foregroundPush.ts");
    ...
    it("schedules a time-sensitive notification that opens NoLate's own screen", () => {
        ...
        expect(scheduler).toContain("NoLateCustomAlarmNotificationContract.payload(");
    ...
    it("registers all NoLate-owned categories and their six foreground-only actions natively", () => {
        const identifiers = [
            ...
            const match = models.match(new RegExp(`${name}\\s*=\\s*\\n?\\s*"([^"]+)"`));
        ...
        expect(models).toContain('legacyCategoryIdentifier = "schedule_depart_now"');
        expect(models).toContain('legacyDepartureActionIdentifier = "schedule_depart_now_action"');
        expect(models).toContain('legacySnoozeActionIdentifier = "schedule_snooze_action"');
    ...
    it("replaces any older preview request instead of accumulating test alarms", () => {
        ...
        expect(scheduler).toContain("removePendingNotificationRequests(withIdentifiers: pending)");
        expect(scheduler).toContain("removeDeliveredNotifications(withIdentifiers: delivered)");
    });
    ...
    it("never creates a new AlarmKit reservation", () => {
        ...
        const scheduleStart = coordinator.indexOf("private func scheduleSystemDeliveryUnlocked(");
    ...
    it("routes every custom action before the legacy direct-mutation action", () => {
        ...
        expect(foregroundRouter.slice(customRouting, legacyMutation))
            .toContain('openCustomAlarmOnce(customAlarmTarget, "interaction")');
    });
    ...
    it("loads simulator local listeners without importing the push-token root index", () => {
        ...
        expect(loader).not.toContain('requireOptionalNativeModule("ExpoPushTokenManager")');
    });
