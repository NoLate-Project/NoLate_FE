...
const scheduleSourcePaths = [
  'src/modules/schedule/hooks/scheduleIndexControllerModel.ts',
  'src/modules/schedule/hooks/useScheduleIndexState.ts',
  'src/modules/schedule/hooks/useScheduleIndexLifecycle.ts',
  'src/modules/schedule/hooks/useScheduleIndexCalendarData.ts',
  'src/modules/schedule/hooks/useScheduleIndexAuxiliaryData.ts',
  'src/modules/schedule/hooks/useScheduleIndexTodayFocus.ts',
  'src/modules/schedule/hooks/useScheduleIndexToolbarActions.ts',
  'src/modules/schedule/hooks/useScheduleIndexDisplayModel.ts',
  'src/modules/schedule/hooks/useScheduleIndexBottomBar.tsx',
...
const sources = Object.fromEntries(
  scheduleSourcePaths.map(path => [path, readFileSync(path, 'utf8')]),
);
const scheduleSource = scheduleSourcePaths
  .map(path => sources[path])
  .join('\n');
const lifecycleSource =
  sources['src/modules/schedule/hooks/useScheduleIndexLifecycle.ts'];
const calendarDataSource =
  sources['src/modules/schedule/hooks/useScheduleIndexCalendarData.ts'];
const auxiliaryDataSource =
  sources['src/modules/schedule/hooks/useScheduleIndexAuxiliaryData.ts'];
const todayFocusSource =
  sources['src/modules/schedule/hooks/useScheduleIndexTodayFocus.ts'];
const toolbarActionsSource =
  sources['src/modules/schedule/hooks/useScheduleIndexToolbarActions.ts'];
...
describe('month agenda loading presentation', () => {
  ...
  test('카테고리 오류 배너가 상단 pill 메뉴를 덮지 않는다', () => {
    const start = scheduleStyleSource.indexOf('toolbarLayer: {');
    const end = scheduleStyleSource.indexOf(
      'scheduleActionPillLayer: {',
      start,
    );
    expect(start).toBeGreaterThanOrEqual(0);
    expect(end).toBeGreaterThan(start);
    const toolbarStyles = scheduleStyleSource.slice(start, end);
  ...
  test('상세 월 swipe는 무거운 선택 commit 전에 native pill을 먼저 갱신한다', () => {
    ...
    expect(scheduleSource).toContain('onDetailMonthPreview={');
    expect(scheduleSource).toContain('handleDetailMonthPreview');
  });
  ...
  test('표시 중인 월이 캐시에 없을 때만 agenda loader를 노출한다', () => {
    ...
    expect(scheduleSource).not.toContain('hasCompletedInitialScheduleLoadRef');
  ...
  test('visible 월 cache hit도 UI를 막지 않고 sliding prefetch edge를 갱신한다', () => {
    ...
    const cacheHitStart = scheduleLoader.indexOf('if (hasVisibleMonthCache) {');
    const cacheHitEnd = scheduleLoader.indexOf(
      normalizeSourceContract('return;\n        }'),
      cacheHitStart,
    );
    expect(cacheHitStart).toBeGreaterThanOrEqual(0);
    expect(cacheHitEnd).toBeGreaterThan(cacheHitStart);
    const cacheHitBranch = scheduleLoader.slice(cacheHitStart, cacheHitEnd);
  ...
  test('foreground 전체 일정 조회 대신 이미 받은 월 범위를 재사용한다', () => {
    const scheduleApiImport = calendarDataSource;
  ...
  test('보조 badge는 active에서만 push/foreground와 10분 safety refresh를 사용한다', () => {
    ...
    expect(auxiliaryRefresh).toContain('AppState.currentState !== "active"');
  ...
  test('일정 mutation은 검색 cache를 즉시 무효화한다', () => {
    ...
    expect(scheduleSource).toContain('searchResultCacheRef.current.clear();');
    expect(scheduleSource).toContain(
  ...
  test('stack형 월 스크롤은 네트워크 기준 월을 160ms trailing coalesce한다', () => {
    ...
    expect(fetchMonthEffect).toContain('}, STACK_MONTH_FETCH_COALESCE_MS);');
