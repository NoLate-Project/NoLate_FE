...
describe("QuickScheduleModal flow", () => {
    ...
    async function completeRouteFromNotification(rerender: () => Promise<void>) {
        await act(async () => {
            renderer!.root
                .findByProps({ accessibilityLabel: "알림 수정" })
                .props.onPress();
        });
        ...
        await act(async () => {
            renderer!.root
                .findByProps({ accessibilityLabel: "빠른 일정 경로 설정" })
                .props.onPress();
        });
        await rerender();
    ...
    test("장소 행은 목적지만 수정하고 알림의 명시적 CTA만 경로 화면을 연다", async () => {
        ...
        await act(async () => {
            renderer!.root
                .findByProps({ accessibilityLabel: "장소 수정" })
                .props.onPress();
        });
        ...
        await act(async () => {
            renderer!.root
                .findByProps({ accessibilityLabel: "일정 미리보기로 돌아가기" })
                .props.onPress();
        });
        await act(async () => {
            renderer!.root
    ...
    test("분석 실패 뒤 모달을 닫지 않고 원문을 수정할 수 있다", async () => {
        ...
        await act(async () => {
            renderer!.root
                .findByProps({ accessibilityLabel: "빠른 일정 입력 수정" })
                .props.onPress();
        });

        expect(
            renderer!.root.findByProps({ accessibilityLabel: "빠른 일정 문장" }).props.value
        ).toBe("7월 17일 오후 8시 서울역 약속");
    });
    ...
    test("경로 설정을 완료하면 적용된 경로의 알림 수정 화면으로 돌아온다", async () => {
        ...
        expect(renderer!.root.findAllByProps({ accessibilityLabel: "빠른 일정 경로 설정" })).toHaveLength(0);
    });
    ...
    test("경로 완료 후 출발 알림은 기본 OFF이고 ON하면 1시간이 기본 선택된다", async () => {
        ...
        const notificationSwitch = renderer!.root.findByProps({ accessibilityLabel: "출발 알림 받기" });
        ...
        expect(
            renderer!.root.findByProps({ accessibilityLabel: "출발 10분 전부터 교통 확인" })
                .props.accessibilityState
        ).toEqual({ selected: false });
        expect(
            renderer!.root.findByProps({ accessibilityLabel: "출발 30분 전부터 교통 확인" })
                .props.accessibilityState
        ).toEqual({ selected: false });
        expect(
            renderer!.root.findByProps({ accessibilityLabel: "출발 1시간 전부터 교통 확인" })
                .props.accessibilityState
    ...
    test("10분·30분·1시간 라디오는 선택 상태를 하나씩 전환하고 적용값을 저장한다", async () => {
        ...
        for (const selectedLabel of radioLabels) {
            ...
            for (const label of radioLabels) {
                expect(
                    renderer!.root.findByProps({ accessibilityLabel: label }).props.accessibilityState
                ).toEqual({ selected: label === selectedLabel });
            }
        }

        await act(async () => {
            renderer!.root
                .findByProps({ accessibilityLabel: "출발 30분 전부터 교통 확인" })
                .props.onPress();
        });
        await act(async () => {
    ...
    test("출발 알림을 OFF로 적용하면 저장 payload에서 알림 설정값을 제거한다", async () => {
        ...
        await act(async () => {
            renderer!.root
                .findByProps({ accessibilityLabel: "출발 10분 전부터 교통 확인" })
                .props.onPress();
            renderer!.root
                .findByProps({ accessibilityLabel: "출발 알림 받기" })
    ...
    test("분석 버튼을 빠르게 연속으로 눌러도 파서 요청은 한 번만 보낸다", async () => {
        ...
        const onAnalyze = jest.fn(() => new Promise<ScheduleParseResult>((resolve) => {
        ...
        const analyzeButton = renderer!.root.findByProps({ accessibilityLabel: "빠른 일정 문장 분석" });
        await act(async () => {
